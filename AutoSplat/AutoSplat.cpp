...
int DrawSegments2Buffer(SEGMENT* pSegments, TIM_FILE* pTIMData)
{
	...
	for(int f = 0; f < 16; f++){
		for(int k = 0; k < 16; k++){
			for (int j = 0; j < 4; j++) {
				for (int i = 0; i < 4; i++) {
					int number = i + j * 64 + k * 4 + f * 256;
					CopyTIM2Buffer(_TIMXPOS(pSegments[index / 16].strTilePolyStruct[index % 16].cTileRef), _TIMYPOS(pSegments[index / 16].strTilePolyStruct[index % 16].cTileRef), _MAPXPOS(number), _MAPYPOS(number), pSegments[index / 16].strTilePolyStruct[index % 16].cRot);
					index++;
				}
			}
		}
